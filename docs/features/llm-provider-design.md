...
本書は、AIレビュー用Review Backendの種類と責任境界、Provider・モデル・接続先の選択、credential、権限制約、実行フロー、セットアップと配布の方針を定義します。
...
- AI reviewを、採点や解答生成ではなく、利用者自身の実装を次の問題で使える知識へ変える振り返りとして提供する。
- 現在のcodeに対する単発の指摘だけでなく、利用者が明示的に選んだ同一問題の提出snapshot、判定、差分を比較できる共通workflowを支える。
- correctness上の疑い、performance上の懸念、readabilityの提案、style上の好み、代替案を区別できるresponse契約を持つ。
- 重要な指摘がない場合に無理な批判を要求せず、少数の優先事項と次の問題へ持ち越せる原則を返せるようにする。
...
| 比較対象の過去code snapshot | 明示選択時だけ送信 | 初期版では同一問題について利用者が選んだ提出に限定する |
| 選択したsnapshot間のdiff | 明示選択時だけ送信 | 試行錯誤と変更理由の振り返りに利用する |
| 問題に対する全実装履歴 | 送信しない | 必要なsnapshotだけを選択し、履歴全体を暗黙に送らない |
...
履歴reviewでは、対象snapshotの件数、提出時刻、verdict、送信するdiffを実行前に確認できるようにする。AlgoLoomのDBに履歴が存在することを、Review Backendへ履歴全体を送信してよいという同意とみなさない。
...
- 対象code hash
- primary対象のsubmission ID
- 履歴reviewで比較したsnapshotのsubmission ID、code hash、verdict。比較しない場合は保存しない
