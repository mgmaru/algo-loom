...
本書は、AlgoLoomにTurso同期を採用する際の候補方式、データの権威、同期・競合・障害・backupの扱い、採用前の検証条件を定義します。
...
**AlgoLoom MVPは標準SQLiteだけを使用する。MVP完了後に同期を検討する場合は、ローカルDBへのcommitを履歴保存の成功条件とし、Turso Syncを同期方式の第一候補として検証する。**
...
採用はSDKのPython対応、耐久性、競合、強制終了、bootstrap、配布wheelを実測して決定する。検証に不合格の場合は同期公開を延期し、必要になった場合だけEmbedded Replica + outboxまたは別Adapterを評価する。その場合も、outbox内の未共有履歴を`HistoryStore`がローカル履歴として読めなければならない。Cloud障害直後の履歴を`show`できない実装は採用しない。
...
同期機能でoutboxが必要になった場合に主に保存するものは次のとおりである。MVPはoutboxを持たない。
...
次の図は、提出履歴を追記専用にして同期競合を減らす考え方を示す過去の最小例であり、MVPの確定Schemaではない。[MVPスコープ](../product/mvp.md)と[Core契約](../architecture/core-contracts.md)に従い、Coreの論理モデルでは少なくともSolveAttempt、FocusInterval、learning milestone、source snapshot、checkpoint、submission operation、verdict observation、AtCoder account identityを分離して設計する。AI review tableはMVPへ含めない。
...
| 保存先 | 位置付け | 方針 |
|---|---|---|
| Tursoとは別のCloud・オブジェクトストレージ | 主なバックアップ先 | 同期機能の公開後は、暗号化した世代別dumpを置く |
...
- [MVPスコープ](../product/mvp.md)と[Core契約](../architecture/core-contracts.md)に従い、SolveAttempt、FocusInterval、learning milestone、snapshot、checkpoint、提出操作、判定観測、account identityを含む追記中心の履歴を実装する。
- UUIDとAtCoder submission IDによる冪等性を実装する。
- マイグレーション方式を決める。
- 明示exportとmigration前の復旧可能な退避を実装する。自動backupと完全なrestore UXはMVP後とする。
