...
6. freshな解き直しのcheckout作成、履歴分離、部分失敗を設計する。
7. submitの部分成功と再確認を設計する。
8. 公式問題・解説ページのbrowser委譲とspoiler確認を設計する。
9. AtCoder認証の初回・期限切れ導線を設計する。
10. P0の利用者検証を行う。
11. P1の待機、確認、診断、用語を改善する。
12. Coreが安定した後に他ユーザーの提出一覧、AI、同期、catalog、ViewerのP2を検証する。MVPの完了範囲は[MVPスコープ](../product/mvp.md)を正とする。
