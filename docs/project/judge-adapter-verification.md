...
[`p0-21`](../verification/judge-adapter/results/2026-08-12-p0-21.md)により、`abc300_a`、`python-cpython`、[`atcoder_v04_integrated.py`](../../scripts/verification/atcoder_v04_integrated.py)を使い、V-03の提出ID取得直後からV-04の判定待ちと最終判定を観測する実サービス検証を1回承認します。この承認で許可する新規提出は最大1件、自動再送と同一承認内の再提出は0回です。本計画変更自体は提出を実行せず、実行結果は次の新しい記録へ保存します。
