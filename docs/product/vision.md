...
AlgoLoomは、ユーザーカスタマイズを行わなくても、開発者が設計した一貫性のある標準UXでCore操作を利用できる状態を維持する。そのうえで、個人差、端末差、外部toolの選択、反復入力の摩擦を減らすために、必要性が確認できた範囲だけをユーザーがカスタマイズできるようにする。

> 核は一つ、周辺は選べる。

- カスタマイズは、製品側が決めるべき標準UXを利用者へ設計させるためではなく、繰り返し発生する個人的・環境的な摩擦を減らすために提供する。
- 表示上の好み、既定の入力、Editor / Viewer等の外部tool、明示的に選ぶ任意Capabilityは、Coreから分離できる範囲でカスタマイズ対象にできる。
