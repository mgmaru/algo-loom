...
本書は、AlgoLoomが目指す学習体験と、Editor非依存、利用者の主体性、ローカルファースト、環境非侵襲性を中心とする製品全体の設計思想を定義します。

## 1. プロジェクトの目的
アルゴリズムという思考の糸を、ターミナル上で丁寧に織り上げるためのローカルファーストCLIツール。
ブラウザの往復を最小限に抑え、特定のエディタやIDEに依存せず、ユーザーが使い慣れた環境でコーディング・テスト・提出・AIレビュー・成長履歴の管理をシームレスに行う学習基盤を構築する。履歴は他者との順位付けではなく、過去の自分の試行、実装、所要時間、理解の変化を振り返るために利用する。
...
ここでいうエディタ非依存は、すべてのEditor固有APIやpluginを個別対応することではない。保存済みの通常fileとCLIだけでCoreを利用できる「Core互換性」と、AlgoLoomから特定toolを起動するAdapterや将来pluginの「公式連携」を分離する。公式連携がないEditorも通常fileを共有できれば編集手段として利用できるが、Core互換性だけを根拠にEditor固有のdiff mode、task runner、Remote拡張まで検証済みとは表示しない。

#### Coreの独立性

- `get`、`test`、`submit`等の主要コマンドは、エディタ連携がなくても利用できる。
- AlgoLoom Coreは、特定エディタのコマンド、設定形式、plugin APIを知らない。
- 編集中sourceの権威はworkspaceへ保存された通常fileとし、未保存bufferやEditor内部状態を推測しない。
...
エディタ、ファイルマネージャ、ブラウザをAlgoLoom自身へ内蔵しない。上部にタブ、下部にターミナルを持つ統合環境は操作の切り替えを減らすが、次の理由で採らない。

- 内蔵エディタを既定にすると、利用者が選んだ編集環境を中心にする原則に反する。任意にすれば内蔵する意味が薄い。
- ターミナルはHTMLを描画しないため、内蔵ブラウザはテキストブラウザ相当の品質に留まる。実際のレンダリングにはデスクトップアプリ化が必要になり、配布方針と環境非侵襲性が成立しなくなる。
- 通常fileとCLIを境界に保つ現在の構成なら、利用者はターミナルマルチプレクサやIDEの統合ターミナルで自分の画面を組み立てられる。内蔵化はこの組み合わせ可能性を失わせる。

一つの画面へまとめたい需要には、AlgoLoomが器を用意するのではなく、利用者が既に導入したツールの構成として応える。詳細は[言語・実行環境の可搬性設計 §8.6](../architecture/language-and-platform-portability.md#86-一つの画面へまとめる場合の案内)を参照する。
...
AlgoLoomは、他者との順位付けや優劣の比較を製品の目的にしない。利用者が過去の自分の実装、試行錯誤、所要時間、理解の変化を振り返り、自分のコーディング力とアルゴリズム構築力を継続的に高めることを中心に据える。

AtCoder本番、バーチャルコンテスト、他者の解法から学ぶことは利用者が自由に選べる有効な学習方法である。この自由を妨げない一方、それらをAlgoLoom内の利用者間ランキング、公開skill score、勝敗へ変換しない。

```mermaid
flowchart LR
    A[今回のSolveAttempt] --> B[実装・時間・判定・差分を記録]
    B --> C[過去の自分と振り返る]
    C --> D[次の一問へ持ち越す原則を得る]
    D --> E[次回のSolveAttempt]

    X[他者の順位] -. 製品内の評価軸にしない .-> B
...
速くACすることだけを成長とみなさない。履歴は、少なくとも次の異なる変化を振り返れる構造にする。

| 観点 | 振り返る例 | 扱い |
|---|---|---|
| 正確さ | ACまでの提出回数、同じ誤りの再発 | 速さと独立して扱う |
| 思考と実装 | checkpoint間の差分、解法やdata structureの変化 | 単一のscoreへ縮約しない |
| 時間 | 開始、最初のsample通過、初回提出、初ACまでの能動時間 | 順位ではなく本人の参考値として扱う |
| 自立度 | hint、解説、AI review等を利用した時点や範囲 | 利用を善悪で評価せず、任意の文脈として扱う |
| 定着 | 解き直し、別言語での再実装、説明可能性 | 同じ問題の別のSolveAttemptとして残す |

#### 製品上の境界

- global leaderboard、利用者間rank、公開skill scoreをCoreへ導入しない。
- 「平均より遅い」等の他者集団との比較を既定表示にしない。
- 時間、提出回数、連続利用日数を、利用者を急かしたり失敗感を与えたりする単一の成績へ変換しない。
- 便利な既定比較は「同じ問題の前回」「初回提出と最新AC」等、利用者自身の履歴を中心にする。
- 他者のcodeや外部解説を参照する場合も、優劣の断定ではなく解法、表現、trade-offを学ぶ材料として扱う。
- 利用者が記録を望まない場合も、時間計測や詳細分析なしでtest、submit、履歴のCore導線を利用できるようにする。
...
#### 自分のsourceの外部公開と標準toolの境界

自分のsourceをGitHub等で公開するかどうかも利用者が決める。ただし、学習資産を持ち出せる自由を、AlgoLoomによるGitHub認証、repository作成、commit、push、visibility変更の自動化として実装しない。

| 段階 | AlgoLoomの役割 | 境界 |
|---|---|---|
| 完全版export | 履歴、時間、判定、snapshot等をversion付き形式で欠損なく持ち出す | 私的な可搬性・退避を主目的とし、そのまま公開する前提にしない |
| 公開候補bundle | MVP後に実需があれば、明示選択された自作sourceだけをallowlist方式でlocalへ切り出す | 公開可能性を保証せず、問題文、sample、履歴、secret等を既定で含めない |
| 外部公開 | 公式手順と生成先を案内できる | Git、GitHub CLI、GitHub Desktop、Web UI等を利用者が操作し、AlgoLoomはcredentialとremote状態を扱わない |

公開候補bundleはprovider非依存とし、GitHub固有のrepository modelをCoreへ持ち込まない。workspace全体や完全版exportを公開用として暗黙に再利用せず、source origin、hash、含有file、除外dataを外部公開前に確認できるようにする。詳細は[公開用solution bundle将来設計](../features/public-solution-bundle-design.md)を参照する。
...
| 対象 | AlgoLoomの役割 | 境界 |
|---|---|---|
| 自分のsource・提出 | snapshotとして保存し、過去の自分と比較する | Coreの学習資産 |
| AtCoderの問題・解説 | 公式ページを明示操作でbrowser表示する | 本文をDB・cache・exportへ保存しない |
| 他ユーザーのAC提出 | MVP後に公式提出一覧をbrowser表示する | 模範解答と評価せず、codeを取得・diff・実行しない |
| hint・解説等の利用文脈 | 将来、利用者が望む場合だけ最小限を記録できる | 利用有無を善悪、rank、scoreへ変換しない |

外部資料のbrowser表示は、test失敗、WA、提出完了等を契機に自動実行しない。未AC時はspoilerになり得ることを示し、開催中または終了状態を確認できない問題では開かない。詳細は[外部学習資料参照設計](../features/external-learning-resources.md)を参照する。

#### 解き直しによる定着

同じ問題の解き直しは、前回を消してやり直す操作ではなく、新しいSolveAttemptとして残す。freshな解き直しでは前回のsourceを自動copyせず、新しいproblem checkoutと組み込みtemplateから始める。前回と今回の時間、milestone、提出、source差分を別々の観点で振り返り、時間短縮だけを成長とみなさない。詳細は[解き直しworkflow設計](../features/revisit-workflow.md)を参照する。
