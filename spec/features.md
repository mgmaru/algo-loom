...
| `SUBMIT` | AtCoderのアカウントを確認し、一件ずつ明示的に提出し、判定を取得時刻付きで記録する | 自動提出、無条件の再送、判定の推測 | `F-SUBMIT-01`〜`04` | [§8](#8-認証提出判定) |
| `REFERENCE` | 公式の問題ページと解説ページのURLを構成し、既定のブラウザへ開く要求を渡す | 本文・画像・他ユーザーのコードの取得と保存、閲覧完了の判定 | `F-REFERENCE-01`, `F-REFERENCE-02` | [§9](#9-外部学習資料f-reference-01-f-reference-02) |
...
| OSのブラウザへ起動要求する | `F-REFERENCE-01`, `F-REFERENCE-02`, `F-PROBLEM-02`の補助動作 |
...
| `429` | 指示された待機時間を尊重し、即時に再試行しない |
| `5xx`・通信失敗 | 上限付きの指数バックオフで再試行する |
| 継続失敗 | 上限に達したら停止し、入力した問題IDまたはURLと次の行動を示す |

- MVPの対象は終了済みのAtCoder Algorithm問題です。AHC、対話的な問題、特殊ジャッジは、安全に処理できない場合は理由を示して停止します。
- CAPTCHA、レート制限、Bot対策を回避しません。
- リクエスト間隔は既定で設け、無効化するオプションを提供しません。
...
| 問題発見 | AtCoder Problems等を既定のブラウザで開く`browse`相当をMVPへ含めるか（[問題選択・カタログ設計 §6.1](../docs/features/problem-selection-and-catalog.md#61-atcoder-problemsから探す)は初期版の導線として提示、[MVPスコープ §3.1](../docs/product/mvp.md#31-mvpに含める能力)と[ロードマップ §2](../docs/product/roadmap.md#2-製品全体のフェーズ)には含まれない） | 設計レビューで解消する |
| 非公式表示 | 「AtCoder公式ではない」旨をCLIのヘルプやバージョン表示でも示すか（[配布方針ガイド §10](../docs/operations/algoloom-distribution.md#10-非公式表示とブランド)は配布物への記載を要求） | 設計レビューで解消する |
