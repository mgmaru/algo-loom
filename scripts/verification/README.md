...
| `atcoder_v04_integrated.py` | V-04合格観測用のV-04準備→V-03→即時V-04統合実行 | 新規提出前に方式Cの本人照合を済ませ、V-03状態の作成直後から同じIDだけを有限ポーリングする。`p0-21`で新規提出最大1件の実サービス検証を許可済み |
...
**このスクリプトは新しい提出を1件行います。[`p0-21`](../../docs/verification/judge-adapter/results/2026-08-12-p0-21.md)で、このV-03→V-04統合再検証に限り新規提出最大1件を許可しています。`SEND_STARTED`相当以降は成功・失敗・応答不明のいずれでも許可を消費し、同じ許可で再提出しません。**
...
- 実行開始時に、`p0-21`で明示承認したこの統合実行の新規提出最大1件であることを人が確認する。
