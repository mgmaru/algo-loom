...
次の図は、提出履歴を追記専用にして同期競合を減らす考え方を示す過去の最小例であり、MVPの確定Schemaではない。[MVPスコープとCore契約](../design/mvp-scope-and-core-contracts.md)に従い、Coreの論理モデルでは少なくともsource snapshot、checkpoint、submission operation、verdict observation、AtCoder account identityを分離して設計する。AI review tableはMVPへ含めない。
...
- [MVPスコープとCore契約](../design/mvp-scope-and-core-contracts.md)に従い、snapshot、checkpoint、提出操作、判定観測、account identityを含む追記中心の履歴を実装する。
