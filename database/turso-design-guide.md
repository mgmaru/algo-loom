...
この採用判断はAlgoLoomの設計判断である。2026年7月16日時点のTurso公式資料では、Embedded Replicaはレガシー方式と位置付けられ、新規の同期用途にはTurso Syncが推奨されている。したがって、実装開始時にはPython SDKの成熟度、耐久性、競合処理を再検証し、採用方式を最終決定する。
...
| Embedded Replicaのローカルレプリカ | 各端末 | Cloud primaryから同期した複製。古い可能性があり、正本ではない |
| Turso SyncのローカルDB | 各端末 | 未push変更を含み、その端末にとっての最新作業状態になる場合がある |
| Cloud未反映のユーザーDB書き込み | 作成端末のoutbox | Cloud書き込みが成功するまでの一時的な回復元。成功後は削除する |
| バックアップ | 正本ではない | Turso Cloudおよび作業端末とは障害領域が異なる保存先へ、暗号化した世代別dumpまたはスナップショットとして置く |

### 2.3. ローカルDBファイルの役割分担

AlgoLoomの初期版でEmbedded Replicaを採用する場合、ローカルレプリカ、outbox、問題カタログは別のDBとして扱う。
...
- Cloud書き込み失敗に備えて、ローカルレプリカとは別の標準SQLiteにoutboxを持つ。
