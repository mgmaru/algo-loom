...
> 対象: 他者またはLLMが書いたcodeを読み、根拠ある仮説と検証によって修正する能力を鍛える将来の独立学習モード
>
> 状態: 将来構想。初期版の実装対象外
>
> 作成日: 2026年7月18日
...
AlgoLoomは、初期版ではAtCoderの問題取得、local test、提出、履歴、差分、任意のAI reviewを確実に実装する。

その基盤が安定した後の将来構想として、他者またはLLMが書いたcodeを読み、症状から原因仮説を立て、検証可能な予測を示し、最小限の変更と回帰testによって修正を確かめる独立モードを検討する。本書ではこの構想を仮に**Repair Lab**と呼ぶ。名称、command、画面、採点方式は確定事項ではない。
...
AtCoderを対象とするCoreとRepair Labは、目的と利用者導線を分離する。
...
Repair Labを有効化していなくても、AtCoderのCore機能を完全に利用できる状態を維持する。Repair Lab固有の教材、設定、依存関係、履歴は、Coreの起動、test、submit、log、show、diffを妨げてはならない。

### 1.2. 自分の過去codeだけに限定しない理由
...
この不確実性を扱うこと自体を、独立した学習価値として設計する。
...
通常のAtCoder Coreでは、学習順序やAIの利用を必要以上に規定しない。Repair Labでは、利用者が構造化された訓練を明示的に選んだ場合に限り、修正前の仮説記録や段階的なhint解放を課すことができる。
...
- AtCoderの提出履歴と混同しないRepair attemptの保存境界を設計する。
...
これらを満たすまでは、Repair Labを製品上の確約機能として扱わず、将来の差別化仮説として検証を続ける。
