...
> 対象: AlgoLoomが扱うユーザーコード、CLI入力、SQLite / Turso、外部コマンド、ターミナル、Neovim、LLM Provider、将来のWeb UI
>
> 状態: 設計方針
>
> 作成日: 2026年7月16日
...
一方、保存したコードは、後からSQLite、外部コマンド、ターミナル、Neovim、LLM Provider、将来のWeb UI等へ渡される。そのため、**コードを無害化して保存するのではなく、コードを未信頼データとして保持し、各出力先との境界で安全に扱う**。
...
| 外部プロセス | コンパイラ、runtime、online-judge-tools、Neovim |
...
- ユーザーが自分で指定した悪意あるコンパイラ、runtime、Neovim pluginの安全性保証
...
DBは安全化装置ではない。DBへ正常に保存できた値でも、後からterminal、Neovim、LLM、HTML等で解釈されれば問題を起こし得る。したがって、保存前の1回だけではなく、各出力境界でその文脈に合った対策を行う。
...
- tokenはOS keyring等を優先し、平文設定を既定にしない。
...
| Command injection | filename、problem ID、command template | 任意command実行 | `shell=False`、argv配列、`--`、path検証 |
...
| Editor経由の影響 | DBから復元したcode | modeline、plugin、autocommandの副作用 | 安全閲覧mode、`nomodeline`、安全な引数渡し |
...
### 6.6. Neovimによる`show` / `diff`

`nvim -R`はbufferを読み取り専用にする機能であり、editor全体をsandbox化する機能ではない。DB由来のcodeにはmodelineに見える文字列が含まれ得て、ユーザー設定、plugin、autocommandもfile open時に動作し得る。
...
`diff`では同じ原則で`-d`と2つの一時fileをargvとして渡す。

`--clean`はユーザーの通常設定やpluginを読み込まないため、AlgoLoomの「使い慣れたNeovim環境」というUXと競合する。初期実装前に次のどちらを既定にするか決定する。

| mode | 利便性 | 安全性 |
|---|---:|---:|
| 安全閲覧mode: `--clean`、`nomodeline` | 通常設定を使えない | 高い |
| ユーザー設定mode: `-R`、`nomodeline` | 通常のthemeやkeymapを使える | plugin等の影響を受ける |

ユーザー設定modeを提供する場合は明示設定とし、読み取り専用がsandboxではないことを説明する。
...
| `show` | DB、temp file、Neovim | 安全なtemp file、`nomodeline`、argv、読み取り専用mode |
| `diff` | DB、2つのtemp file、Neovim | `show`と同じ要件、各pathを独立検証 |
...
- `show` / `diff`の`nomodeline`と安全閲覧modeの方針決定
...
- Vim / Neovim modelineに見えるcomment
...
9. 個人アプリであっても、parameter bind、`shell=False`、path検証、表示時escapeは過剰対策ではなく、初期品質の一部とする。
