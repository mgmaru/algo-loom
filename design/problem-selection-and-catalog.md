...
Phase 2ではAtCoder Problemsの公開JSONを端末ごとにキャッシュする。Phase 3では、そのカタログを使う`algoloom pick`を追加し、ターミナル内でも問題を選べるようにする。

中心方針は次のとおりである。

1. 問題を探す主経路は、初期版ではAtCoder Problemsとする。
2. 問題を開始する正式な入口は`algoloom get`とする。
...
| 原子的更新 | 新データの準備完了後に一度で差し替え、途中状態を見せない更新方法 |
...
```bash
algoloom get abc300_a
```

または、AtCoder公式問題URLを指定する。

```bash
algoloom get https://atcoder.jp/contests/abc300/tasks/abc300_a
```
...
6. 新しい一時SQLiteカタログを構築する
7. 整合性チェックを行う
8. catalog.sqliteを原子的に差し替える
9. last_success_at、ETag等を更新する
10. 失敗した場合はlast-known-goodを維持する
```
...
| JSON形式変更 | 新データを拒否し、旧版を維持 |
| 件数が異常に減少 | 警告して自動差し替えを停止 |
| Difficultyだけ取得失敗 | 基本問題一覧を利用し、Difficultyなしで表示 |
...
| コマンド | 役割 |
|---|---|
| `algoloom browse` | AtCoder Problemsの問題一覧をブラウザで開く |
| `algoloom browse --user USER` | 指定ユーザーの一覧を開く |
| `algoloom get PROBLEM_ID` | 問題IDから開始する |
| `algoloom get ATCODER_URL` | AtCoder公式URLから開始する |

### Phase 2

| コマンド | 役割 |
|---|---|
| `algoloom catalog status` | 最終確認・更新時刻、件数、エラーを表示する |
| `algoloom catalog update` | カタログを明示更新する |
| `algoloom catalog clear` | ユーザー履歴を残してカタログだけ削除する |
| `algoloom catalog doctor` | スキーマ、件数、ETag、SQLite整合性を診断する |

### Phase 3

| コマンド | 役割 |
|---|---|
| `algoloom pick` | インタラクティブに問題を選んで開始する |
| `algoloom pick --difficulty MIN..MAX` | Difficultyで絞り込む |
| `algoloom pick --contest abc` | コンテスト種別で絞り込む |
| `algoloom pick --status local-unsolved` | AlgoLoomの履歴上で未ACの問題に絞る |
...
```mermaid
flowchart TD
    P1[Phase 1<br/>AtCoder Problemsで発見] --> G[共通のalgoloom get]
    P2[Phase 2<br/>ローカルカタログ] --> P3[Phase 3<br/>algoloom pick / fzf]
    P3 --> G
    G --> A[AtCoder公式確認]
    A --> W[学習ワークスペース]
```
