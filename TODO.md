...
| [`TD-32`](#td-32-テスト方針の骨格を決める) | 機能設計 | テスト方針の骨格を決める | `TD-17`, `TD-18` | 未着手 |
...
| [`TD-35`](#td-35-実装準備の要件と完了条件をまとめる) | 環境整備 | 実装準備の要件と完了条件をまとめる | `TD-30`, `TD-32`, `TD-33`, `TD-34` | 未着手 |
...
**目的:** `spec/features.md` §13 の品質要件`Q-01`〜`Q-15`と §15 の受け入れシナリオ`E2E-01`〜`E2E-30`には、検証観点と合格条件はありますが、**どの手段で確認するか**がありません。`TD-17`は`LanguageProfile`と`HostPlatform`の契約テストだけ、`TD-18`は環境の確保だけを扱います。45項目の割り当てが無いまま実装へ渡すと、テストの範囲と外部作用の扱いを実装側が決めることになります。

**手順:**

1. テストの種別を定義する。少なくとも、契約テスト、単体、結合、障害注入、実機確認、利用者検証を区別する。
2. `Q-01`〜`Q-15`と`E2E-01`〜`E2E-30`の45項目へ、種別と確認手段を1つ以上割り当てる表を作る。`TD-17`の契約テスト項目と重複させず、対応付ける。
...
6. AtCoderのページ構造の変更を検知する方法を決める。定期的な自動アクセスは[§7.2](#72-実行時に行ってはならないこと)の「背後でのクロールを行わない」に抵触するため、人が明示的に実行する確認として設計する。
7. 実装が完了したと判定する時点を、[MVPスコープ §5](docs/product/mvp.md#5-mvp完了条件)の完了条件と対応付ける。
8. テストランナー、ディレクトリ構成、CIの設定は決めない。`TD-26`と工程5で扱う。

**完了条件:**

- [ ] `Q-01`〜`Q-15`と`E2E-01`〜`E2E-30`の45項目すべてに確認手段が割り当てられている
...
4. 擬似ジャッジが返す応答の範囲を決める。提出の受理、提出IDの発行、判定待ち、各判定、拒否、レート制限、通信断を少なくとも含める。
5. 擬似ジャッジを製品コードから分離する境界を決める。テスト専用とし、配布物へ含めない。
6. [`scripts/verification/`](scripts/verification/)の既存コードを流用しないことを明記する。[§7.2](#72-実行時に行ってはならないこと)の「検証支援コードを製品コード、CI用の認証手段として扱わない」と整合させる。
7. フィクスチャへ秘密情報、実際のアカウント名、実Cookieが混入しないことを機械的に確認する要件を、`TD-35`へ渡す。
...
| 実装タスクの分解 | [`spec/features.md`](spec/features.md) §14 の11単位を、一度の作業として着手できる粒度へ分ける。各タスクに完了条件と対応する受け入れシナリオを付ける |
...
3. `TD-33`から渡された、秘密情報の混入を機械的に確認する要件を反映する。
4. `docs/project/implementation-prep.md`を実装リポジトリへ複製する時期と、複製後は設計リポジトリ側を更新しない旨を書く（[§0.4](#04-工程5実装準備の扱い)）。
5. 工程5の完了をもって工程6「実装」を開始できることを書く。

**完了条件:**

- [ ] 工程5で作る成果物の一覧と、成果物ごとの完了条件がある
- [ ] 作り方（ツール名、ファイル名、ワークフローの構成）を確定していない
- [ ] `TD-32`、`TD-33`、`TD-34`の結論が反映されている
