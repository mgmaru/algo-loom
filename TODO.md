...
| 5 | 実装 | 実装リポジトリのコード | 未着手 | 対象外（リポジトリの作成と`spec/`の同期までは`TD-30`で行う） |
...
| 完了 | 同§4の実装開始条件10項目を満たし、`spec/`を実装リポジトリへ同期した時点。確認作業は[`TD-28`](#td-28-実装開始条件の充足を確認する)〜[`TD-30`](#td-30-実装リポジトリを作成し同期方式を決める) |
...
| [`TD-26`](#td-26-各ライブラリを選定する) | 設計判断 | 各ライブラリを選定する | `TD-14`, `TD-25` | 未着手 |
...
8. 実装中に判明した設計変更を、この設計リポジトリへ戻す手順を記録する。
...
本書は8つの文書を新規作成します。作る前に、既存の[Core契約](docs/architecture/core-contracts.md)または[`spec/features.md`](spec/features.md)へ追記できないか確認します。同じ事実を複数の文書で独立に決めると、後から食い違います。
...
| 対象 | 理由 | 再開する条件 |
|---|---|---|
| 実装コードの作成 | 実装開始条件10項目を満たし、`spec/`を同期するまで開始しない | `TD-30`の完了後 |
