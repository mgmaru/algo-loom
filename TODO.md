...
| 完了 | 同§4の実装開始条件10項目を満たし、`spec/`を実装リポジトリへ移せる状態にした時点。確認作業は[`TD-25`](#td-25-実装開始条件の充足を確認する)と[`TD-26`](#td-26-specを実装リポジトリへ移せる状態にする) |
...
| [`TD-26`](#td-26-specを実装リポジトリへ移せる状態にする) | 文書整備 | `spec/`を実装リポジトリへ移せる状態にする | `TD-24`, `TD-25` | 未着手 |
...
4. 10条件すべてを満たしたら、`TD-26`へ引き渡す。

**完了条件:**

- [ ] 10条件すべてに成果物が対応付いている
- [ ] 不足があれば追加のTODOが本書へ追記されている
- [ ] 10条件を満たしたことを確認し、`TD-26`へ引き渡している

---

#### `TD-26` `spec/`を実装リポジトリへ移せる状態にする
...
4. 未決事項の記述を、参照ではなく「未確定である事実」と「確定するまでの安全な既定動作」へ書き換える。`TD-25`の時点で未確定のまま残る項目だけが対象になる。
...
| 対象 | 時期 |
|---|---|
| これから`spec/`へ書く内容（`TD-14`〜`TD-24`） | 最初から§6.1に従う |
| 既存の`spec/`にある`docs/`参照 | 実装リポジトリへ同期する直前に一度で整理する（[`TD-26`](#td-26-specを実装リポジトリへ移せる状態にする)） |

先に既存分を整理しても、機能設計の追記で参照がまた増えます。新規分だけ規則を守り、既存分は最後にまとめて整理します。

### 6.3. 新しい文書を増やす前に既存文書を確認する

本書は5つの文書を新規作成します（論理モデル、契約テスト仕様、中断点と回復経路、CLI契約、ライブラリ選定記録）。作る前に、既存の[Core契約](docs/architecture/core-contracts.md)または[`spec/features.md`](spec/features.md)へ追記できないか確認します。同じ事実を複数の文書で独立に決めると、後から食い違います。
